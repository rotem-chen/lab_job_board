...
* **GitHub Repository URL:** https://github.com/rotem-chen/lab_job_board
* **Screenshot 1:** Running application at http://localhost - `![App](localhost_app_running.png)`
* **Screenshot 2:** All containers healthy - `![Healthy](terminal_containers_healthy.png)`
* **Screenshot 3:** Successful GitHub Actions pipeline - `![Pipeline](github_actions_success.png)`
* **Screenshot 4:** Docker Hub repository - `![DockerHub](docker_hub_repositories.png)`
...
jobs-service
    jobs-service: Before 274MB -> After 274MB (Layer count reduced).

    The image size remained approximately the same because the Python source code is very lightweight. However, the layer count was successfully reduced by chaining the apt-get commands with && and removing the redundant chown layer."

applications-service
    applications-service: Before 223MB -> After 216MB.
frontend 
    Before 98MB -> After 98MB.
...
3. Which upstream container receives the request and on which port:
   Nginx forwards the transformed request to the upstream `applications-service` container over the internal Docker network (`jobboard-network`) on its internal port `3001`.
...
4. How the response travels back to the browser:
   The `applications-service` processes the POST request (e.g., saving data to the database) and generates an HTTP response. It sends this response back to the `nginx-proxy` container via the internal Docker network. Finally, Nginx passes the response back out through localhost on port 80 directly to the user's browser.
    ...
    Command:
    docker exec -it jobs-service python3 -c "import psycopg2; import os; conn = psycopg2.connect(os.environ['DATABASE_URL']); print('Connected to PostgreSQL:', conn.get_dsn_parameters()); conn.close()"
...
Updated docker-compose.yml to use Docker secrets. Modified db.js (Node.js) and database.py (Python) to read from /run/secrets/db_password and dynamically encode the password into the DATABASE_URL using encodeURIComponent and urllib.parse.quote_plus."
